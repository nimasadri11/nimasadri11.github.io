...
developer_message = {
    "model_instructions":   "You are Codex...",
    "permission_policy":    sandbox_rules,
    "memory_summary":       load("memory_summary.md")[:5000],
    "collaboration_mode":   "Plan | Execute | Default",
    "personality":          "Friendly | Pragmatic | None",
    "skill_summaries":      loaded_skills,
    "plugin_summaries":     loaded_plugins,
}
user_message = {
    "agents_md":            walk_agents_md(root, cwd),
    "env":                  {cwd, shell, date, tz, network},
}`}
...
**Claude Code** re-sends the full user context on every turn (it's memoized, so it doesn't recompute), but injects deltas for specific changes as `<system-reminder>` messages: new MCP servers, new agents, newly discovered deferred tools, and file attachment results. The session memory subagent (covered in Chapter 6) also writes running notes to a markdown file during the session; these survive context compaction.
